...
static int ad53x8_spi_write_val(struct spi_device *spi, u8 addr, u16 val, u8 shift)
{
	...
	msg = cpu_to_be16((0 << 15) | (addr << 14) | (val << shift));

	return spi_write(spi, &msg, sizeof(msg));
}

static int ad53x8_spi_write_cmd(struct iio_dev *indio_dev,
				...
				u8 chan, u8 val)
{
	...
	msg = cpu_to_be16(msg);
	return spi_write(st->us, &msg, sizeof(msg));
}

static int ad53x8_get_channel_vref(struct ad53x8_state *st,
				   ...
				   long mask)
{
	...
	case IIO_CHAN_INFO_SCALE:
		scale_mv = ad53x8_get_channel_vref(st, chan->channel);
		if (scale_mv < 0)
			return scale_mv;
		*val = scale_mv;
				...
				long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		return ad53x8_spi_write_val(st->us, chan->address, val, chan->scan_type.shift);
...
static int ad53x8_reset(struct iio_dev *indio_dev,
	const struct iio_chan_spec *chan, unsigned int mode)
{
	return ad53x8_spi_write_cmd(indio_dev, RESET, chan->address, mode);  // TODO
...
static const struct iio_enum ad53x8_gain_enum = {
    .items = ad53x8_gain_modes,
    .num_items = ARRAY_SIZE(ad53x8_gain_modes),
    .set = ad53x8_set_gain,
};
	...
	size_t len)
{
	...
	ret = ad53x8_spi_write_cmd(indio_dev, PDOWN, chan->channel, (u8)pwr_down);
...
static const struct iio_chan_spec_ext_info ad53x8_ext_info[] = {
	...
	IIO_ENUM_AVAILABLE("ldac", &ad53x8_ldac_enum),
	IIO_ENUM("gain_2vref", IIO_SHARED_BY_TYPE, &ad53x8_gain_enum),
	IIO_ENUM_AVAILABLE("gain_2vref", &ad53x8_gain_enum),
	IIO_ENUM("reset", IIO_SHARED_BY_TYPE, &ad53x8_reset_enum),
	IIO_ENUM_AVAILABLE("reset", &ad53x8_reset_enum),
	{ },
};
...
static int ad53x8_probe(struct spi_device *spi)
{
 ...
 //
	...
	ret = devm_regulator_bulk_get(&spi->dev, 2, st->vref_reg);
	if (ret) {
...
error_disable_reg:
	regulator_bulk_disable(2, st->vref_reg);;
...
static int ad53x8_remove(struct spi_device *spi)
{
	struct iio_dev *indio_dev = spi_get_drvdata(spi);
	struct ad53x8_state *st = iio_priv(indio_dev);

	iio_device_unregister(indio_dev);
	return regulator_bulk_disable(2, st->vref_reg);
}
